 ...
 */
function prevImg(fileTypeMsgArray){
    initBigImgDeg();
    currentFileIndex-=1;
    if(currentFileIndex<0){ 
        currentFileIndex=fileTypeMsgArray.length-1;
    }
    showFile(currentFileIndex,fileTypeMsgArray);
}
 ...
 * @param {*} fileTypeMsgArray 循环下标对应的数组
 */
function nextImg(fileTypeMsgArray){
    initBigImgDeg();
    currentFileIndex+=1;
 ...
 */
function showFile(index,fileTypeMsgArray){
    ...
    if(fileType=="jpg" || fileType=="png"|| fileType=="jpeg"|| fileType=="tiff"|| fileType=="bmp"|| fileType=="raw"){  //============显示图片==============================
        $(".JPGdom,.transform-img").removeClass("hidden");  //图片容器和旋转图片的按钮隐藏
        $(".JPGdom img").attr("src",fileTypeMsgArray[index].src);
        ...
        // pdfURL
        if(fileId){
            PDFObject.embed(fileTypeMsgArray[index].src, "#PDFdom",options);
        }else{
            PDFObject.embed("/cpSearch/file/down?isDown=NO&fileId=0&fileName=a.pdf&fileURL="+fiteMsg.fileDownloadPath, "#PDFdom",options);
        }
    }else if(fileType=="word" || fileType=="excel" || fileType=="xlsx" || fileType=="xls"){    //===========显示word+excle表格==============================
         $(".Otherdom").removeClass("hidden");
         $(".Otherdom iframe").attr("src",fileTypeMsgArray[index].src);
        ...
        audio.src=fileTypeMsgArray[index].src;
        audio.load();
        $(".img-ctrl .downLoadMp3").attr("href","/cpSearch/file/down?isDown=YES&fileId="+fileId+"&fileName="+fileName);
    }else if(fileType.toLocaleLowerCase()=="mp4" || fileType.toLocaleLowerCase()=="ogg"){
        $(".videoDiv,.downLoadMp3").removeClass("hidden");
        let video=document.getElementsByTagName('video')[0];
        video.src=fileTypeMsgArray[index].src;
        video.load();
        $(".img-ctrl .downLoadMp3").attr("href","/cpSearch/file/down?isDown=YES&fileId="+fileId+"&fileName="+fileName);
        ...
        if (userAgent.indexOf("Chrome") > -1){ //判断是否chorme浏览器
            ...
            if(isDownLoad){
                window.location=fileTypeMsgArray[index].src
            }
        }
    }else{  
        $(".discern").removeClass("hidden");
        ...
        let isDownLoad=confirm('sorry，暂不支持预览'+fileType+'格式文件！，是否需要下载该文件到本地？');
        if(isDownLoad){
            window.location="/cpSearch/file/down?isDown=YES&fileId="+fileId+"&fileName="+fileName
        }
    }
}
